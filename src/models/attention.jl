...
function (mha::MultiheadAttention)(x::A) where {T,A<:AbstractArray{T}}
    ...
    Q = permutedims(reshape(Q, :, dh, mha.nhead, size(x, ndims(x))), [2, 1, 3, 4])
    K = permutedims(reshape(K, :, dh, mha.nhead, size(x, ndims(x))), [2, 1, 3, 4])
    V = permutedims(reshape(V, :, dh, mha.nhead, size(x, ndims(x))), [2, 1, 3, 4])
